print("\n" * 3)                  #Starting after 3x empty lines.
...
def def_default():
    global list_foods, list_drinks, list_services, list_item_order, list_item_price     
    ...
    while True:                                            #Repeat Menu until stops.
        ...
        if (len(input_1) == 1):                                           #Checking input length.
            if (input_1 == 'O'):                                          #If input is "O".
                ...
                break                                                     #Stop repeating Main Menu.
            elif (input_1 == 'C'):                                        #If input is "C".
                print("\n" * 3)                                         #Create 100 empty lines.
                def_order_cancel()                                        #Start Order cancel function.
                break                                                     #Stop repeating Main Menu.
                ...
                print("\n" * 3 + "ERROR: Invalid Input (" + str(input_1) + "). Try again!")     #Invalid input.
    ...
    while True:                                             # While looping to keep menu alive
        ...
        if len(input_1) == 1:
            if (input_1 == 'F'):  #Easy Access Checking Logic
                ...
                print("\n" * 3 + "ERROR: Invalid Input (" + str(input_1) + "). Try again!") # Handling Bad Inputs
        else:
...
def def_full_file_reader():                                                                         #mustafa
    ...
    while i <= (len(list_foods) - 1): #Enumarte through food list to filter out prices and setup print Formatting by replacing spaces with count difference of string length and align Prices to the most left of the terminal
        if 'RM' in list_foods[i]:
            list_foods[i] = str(list_foods[i][:list_foods[i].index('Ksh') - 1]) + ' ' * (20 - (list_foods[i].index('Ksh') - 1)) + str(list_foods[i][list_foods[i].index('RM'):])
        i += 1

    i = 0
    while i <= (len(list_drinks) - 1):
        if 'RM' in list_drinks[i]:
            list_drinks[i] = str(list_drinks[i][:list_drinks[i].index('Ksh') - 1]) + ' ' * (20 - (list_drinks[i].index('Ksh') - 1)) + str(list_drinks[i][list_drinks[i].index('RM'):])
        i += 1

    i = 0
    while i <= (len(list_services) - 1):
        if 'RM' in list_services[i]:
            list_services[i] = str(list_services[i][:list_services[i].index('Ksh') - 1]) + ' ' * (20 - (list_services[i].index('Ksh') - 1)) + str(list_services[i][list_services[i].index('RM'):])
        i += 1
...
def def_file_sorter(): # Applying Sorting to the array to be sorted from A-Z ASC ((AND)) Extracting out prices after sorting and appending them to a prices array accordingly to a parrallel indexes
    ...
    while i < len(list_foods):
        list_item_price[i] = float(list_foods[i][int(list_foods[i].index("Ksh") + 3):]) # Extracting Out "Kshs" + [SPACE] from and cast out the string into an integer
        i += 1

    i = 0
    while i < len(list_drinks):
        list_item_price[40 + i] = float(list_drinks[i][int(list_drinks[i].index("Ksh") + 3):]) # Applying extraction on 40 and above items which are the drinks
        i += 1

    i = 0
    while i < len(list_services):
        list_item_price[80 + i] = float(list_services[i][int(list_services[i].index("Ksh") + 3):]) # Applying extraction on 80 and above items wich are Services
        i += 1
def_file_sorter()
...
def def_food_drink_order():
    while True:
            ...
            while i < len(list_foods) or i < len(list_drinks):
                var_space = 1
                if i <= 8:                      # To fix up to space indention in console or terminal by applying detection rule to figure out spacing for THREE DIGITS numbers
                    var_space = 2
            ...
            print("\n (M) MAIN MENU             (P) PAYMENT             (C) CANCEL                  (E) EXIT\n" + "_" * 72)
            ...
            if (input_1 == 'C'):
                print("\n" * 3) 
                def_order_cancel() # Return to main menu by calling it out
                break
            if (input_1 == 'P'):
                print("\n" * 3)
            ...
            try:        #Cautions Error Handling to prevent program crashing and hand out exceptions as a readable error to notify user
                ...
                if ((int(input_1) <= len(list_foods) and int(input_1) > 0) or (int(input_1) <= len(list_drinks) + 40 and int(input_1) > 40)):
                     ...
                     if int(input_2) > 0:
                        ...
                        print("\n" * 3 + "ERROR: Invalid Input (" + str(input_2) + "). Try again!")
            except:
                print("\n" * 3 + "ERROR: Invalid Input (" + str(input_1) + "). Try again!")
